CREATE VIEW vw_satilik_ilanlar
AS
SELECT
    ...
    tapu_durumlari.durum_adi AS [Tapu Durumu],

    CASE
        WHEN satilik_ilanlar.krediye_uygun_mu = 1 THEN 'Evet'
        ELSE 'Hayır'
    END AS [Krediye Uygun Mu],
...
FROM ilanlar
INNER JOIN satilik_ilanlar
    ON ilanlar.id = satilik_ilanlar.ilan_id
...
INNER JOIN tapu_durumlari
    ON satilik_ilanlar.tapu_durumu_id = tapu_durumlari.id
